...
```{r}
for (id in 0:137) { # change 137 to last page
  
  # Initializing new home page link based on PM id
  pm_home_link <- paste0("https://www.pm.gov.au/media?body_value=&field_media_type_value=transcript&page=",0 # from link selector
  )
...
for(i in 1:11) {
  ...
  #colMeans(res)[[1]] -> c_v
  coherance_sc[i,3] = colMeans(res)[[1]] #mean_logratio
  coherance_sc[i,4] = colMeans(res)[[2]] #mean_pmi
  coherance_sc[i,5] = colMeans(res)[[3]] #mean_npmi
  coherance_sc[i,6] = colMeans(res)[[4]] #mean_difference
  coherance_sc[i,7] = colMeans(res)[[5]] #mean_npmi_cosim
  coherance_sc[i,8] = colMeans(res)[[6]] #mean_npmi_cosim2
}
